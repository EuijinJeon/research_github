...
> ⚠️⚠️ **2026-08-19 3차 정정 — §12 를 먼저 볼 것.**
> 이 표가 "영역 병합 계열 vs 파라미터 분해 계열" 을 **대등하게** 그린 것 자체가 틀렸다.
> 실제로는 영역 계보 **안에서** 갈린다 — 추출·세기·귀속은 활발하고(89~98 인용),
> **병합만 죽었다(2~20 인용).** 우리가 하던 것이 하필 그 죽은 가지다.
> 정확한 구도는 **§12.4** 의 표를 볼 것. 새 방향은 **§13**.
>
...
## 12. ⚠️⚠️ 검증: "비슷한 선형함수끼리 묶기" 는 주류가 아니다 — 병합만 죽었다

> **계기 (2026-08-19, 사용자 지적):** *"저건 activation function이 relu라는거에 강하게 의존하고,
> 조각 하나 크기가 작아질수록 정의역도 작아질꺼라 선형함수 꼴 자체의 중요성도 떨어질 것"*
>
> 세 가지를 확인했고 **셋 다 맞았다.** ②에는 중요한 단서가 붙는다.

### 12.1 ReLU 의존 — polytope lens 저자들이 직접 적어뒀다 [원문]

미해결 질문 목록의 **첫 항목**이 이것이다:

> **Fuzzy polytope boundaries with other activations** — For the sake of simplicity, we've been
> assuming ... piecewise linear activation functions such as ReLU. But many networks today,
> including large language models, often use smooth activations such as GELU and softmax, which
> mean that **their polytopes won't really be polytopes** — their edges will be curvy or even 'blurred'.

트랜스포머 확장도 미해결로 남겼다.
→ **조각·폴리토프라는 어휘 전체가 ReLU 계열 전용이고, 그 한계가 2022년에 이미 공개돼 있었다.**

### 12.2 조각이 작다 — 우리 데이터로 쟀다. **MNIST에서 치명적, 2D에서는 아님** ★

데이터 점에서 무작위 방향으로 영역을 벗어나기까지의 거리를 최근접이웃 거리와 비교했다
(`walkthrough.py` STEP 12, 표본 200개 기준):

| 세팅 | 영역 반경 | 최근접이웃 거리 | 비 | 영역당 점 |
|---|---|---|---|---|
| moons `h=[64]` | 0.064 | 0.025 | **2.6배** | 7.0 |
| spiral `h=[64]` | 0.060 | 0.009 | **6.7배** | 9.2 |
| **MNIST `h=[128]`** | **1.04** | **15.6** | **0.067배** | **1.0** |

- **2D:** 영역이 데이터 간격보다 3~7배 넓다. `[A_r|b_r]` 가 **진짜로 이웃을 기술**한다.
- **MNIST:** 영역이 데이터 간격의 1/15 이다. 영역 안에 자기 점 하나뿐이고 다른 데이터는
  **절대 안 들어온다.** 즉 `[A_r|b_r]` 는 "영역 위의 선형함수" 가 아니라 **그 점의 야코비안**이다.
  7,850개 숫자로 점 하나를 설명하는 셈 — **설명이 설명 대상보다 길다.**

→ **MNIST 영역 클러스터링 = 점별 야코비안 클러스터링.**
  §4 에서 정한 D3 방침(`Pruner` 로 6만 개 전부 처리)은 **계산은 되지만 해석이 안 된다.**
  ⚠️ **D3 취소.** (`open_questions.md` D3 참조)

→ **I3 의 진짜 의미가 이것이었다.** "영역 클러스터링 = 데이터 클러스터링" 이라고 완곡하게
  적고 "그러니 함수+분포의 성질" 로 넘겼는데, 정확히는 **MNIST 에서 이 방법이 성립하지 않는다**는 뜻이었다.

### 12.3 인용수가 계보 **안에서** 갈린다 [OpenAlex, 2026-08]

| 무엇을 하는 논문인가 | 논문 | 연도 | 인용 |
|---|---|---|---|
| **영역 세기 · 이론** | Hanin & Rolnick | 2019 | **98** |
| **영역별 피처 귀속** | OpenBox | 2018 | **89** |
| 영역 **병합** | Aletheia | 2020 | 20 |
| 영역 **클러스터링** | polytope lens | 2022 | 5 |
| 영역 **클러스터링** | Clustering-Based Interpretation of Deep ReLU Network | 2021 | **2** |

(arXiv DOI 레코드는 출판본과 분리돼 낮게 나오므로 제목 검색으로 정본을 잡았다.
 절대값보다 **같은 방법으로 잰 비율**을 볼 것.)

→ **"영역" 이 죽은 게 아니라 "병합" 이 죽었다.** 4~50배 차이는 잡음이 아니다.
→ §9 에서 확인한 2024~2026 논문들(AffineLens, Expressivity Saturation, Region Seeding)도
  **전부 세기만 하고 병합하지 않는다.** 병합 계열은 §9 의 "세 갈래" 어디에도 없었다.

### 12.4 그래서 §1 의 "두 계열" 표를 다시 그려야 한다

1차 조사는 "영역 병합 계열 vs 파라미터 분해 계열" 로 대등하게 그렸다. **실제 구도는 이렇다:**

| 갈래 | 하는 일 | 상태 |
|---|---|---|
| 영역 **추출·세기·기하** | 몇 개인가, 어떻게 생겼나 | ✅ **활발** (98 인용 + 2024~2026 신작 3편) |
| 영역별 **귀속** | 이 입력에서 어느 피처가 기여했나 | ✅ **활발** (89 인용, Re3 2026) |
| 영역 **병합** (우리가 하던 것) | 비슷한 것끼리 묶어 설명을 줄이자 | ❌ **사실상 종료** (2~20 인용) |
| **파라미터 분해** | 가중치를 컴포넌트로 쪼개자 | ✅ **활발** (APD→SPD→VPD) |
| **활성 영역 분해 (soft)** | 활성공간을 영역+국소부분공간으로 | ✅ **신흥** (MFA 2026, SAE 압도) |

**우리 Stage 1 의 전반부(추출·세기·볼록성)는 건강한 갈래에 있었고, 후반부(병합)만 죽은 갈래였다.**

### 12.5 무엇이 남고 무엇을 버리는가

| STEP 6~11 에서 한 것 | 판정 |
|---|---|
| 병합 대상으로서의 조각 클러스터 | **버림** |
| 거리 척도 비교 | **남음** — 무엇을 잴지 고르는 문제는 어디에나 있다 |
| 손잡이(λ)의 경계 찾기 | **남음** — STEP 9 의 "조용히 틀림" 은 일반 현상 |
| 앵커 | **남음** — 선행연구가 필수로 규정 |
| 랜덤 배정 대조군 | **남음** — 곡선 하나는 아무 말도 안 한다 |
| greedy = 상계 명시 | **남음** |
| 비용함수 함정 | **남음** |

→ **틀린 것은 "무엇을 묶을까" 였지 "어떻게 검증할까" 가 아니었다.**

→ 그리고 **STEP 8~11 이 이미 균열을 보이고 있었다:**
  네 척도 중 둘이 앵커 탈락(STEP 8) / 손잡이를 조금 돌리자 greedy 가 조용히 틀림(STEP 9) /
  랜덤 배정 대비 이득이 겨우 1.67배(STEP 11).
  **방법이 대상을 감당 못 한다는 신호가 계속 나왔는데 읽지 못했다.**

---

## 13. 새 방향: bilinear MLP — 가중치만으로 하는 MLP 분해

> **선정 기준 (사용자, 2026-08-19):** *"ML의 구조를 이해하는 것이라는 목적을 공유하는
> 다른 선행연구를 찾아서 그걸 따라가는 걸 목적으로 해."*
> → 죽은 갈래를 또 고르지 않도록 **활력을 먼저 확인**했다.

**Bilinear MLPs enable weight-based mechanistic interpretability**
(Pearce, Dooms, Rogers, Oramas, Sharkey — **ICLR 2025 Spotlight**, `arXiv:2410.08417`)

계보가 있다: `2305.03452`(technical note, Sharkey 2023) → `2406.03947`(weight-based decomposition)
→ `2410.08417`(ICLR'25 Spotlight). **PDF 3편 모두 `external/papers/` 에 받아뒀다.**

### 13.1 왜 이것인가 — 우리 프로젝트와 목적이 같다

논문 초록 첫 문장:

> **A mechanistic understanding of how MLPs do computation in deep neural networks remains elusive.**
> Current interpretability work can extract features from hidden activations over an input dataset
> but **generally cannot explain how MLP weights construct features.**

서론:

> MLPs have previously been treated as **undecomposable components** in interpretability research.

**우리 CLAUDE.md 의 "MLP 해석가능성" 과 같은 문제를 건다.**
게다가 SAE 계열 비판까지 같은 자리에 있다 — *"SDL 기반 접근은 어떤 피처가 있는지만 말할 뿐,
그것이 어떻게 형성되는지는 말하지 않는다."*

### 13.2 §12 의 세 근거를 **전부** 무력화한다

| §12 의 근거 | bilinear 에서는 |
|---|---|
| ① ReLU 전용 | element-wise 비선형성이 **아예 없다.** GELU/softmax 문제가 발생하지 않고, 트랜스포머 MLP 의 drop-in 대체가 된다 |
| ② 정의역이 작다 | 고유벡터는 **전역**이다. 입력 데이터가 아예 필요 없다 — "영역 크기" 라는 개념 자체가 없다 |
| ③ 주류가 아니다 | **ICLR'25 Spotlight** |

### 13.3 무엇을 하는가

bilinear layer 는 `g(x) = (W_1 x) ⊙ (W_2 x)` 꼴이라 **3차 텐서 하나로 완전히 표현된다.**
그 텐서를 출력 방향으로 잘라 **고유분해**하면, 얻은 고유벡터들이
**층의 원래 계산과 정확히 등가**인 분해가 된다 (근사가 아니다).

- 고유벡터 활성은 입력에 대해 **2차** — `λ_i (v_i^T x)^2`
- MNIST/Fashion-MNIST 단층 모델에서 상위 고유벡터가 **클래스 구성요소 / 엣지 검출기**로 읽힌다
- 그 고유벡터로 **적대적 예제를 만들어** 인과적 중요성을 검증한다

### 13.4 우리 워크스루와 정확히 이어진다 ★

STEP 13~14 가 이미 다리를 놓아뒀다:

| | 분해가 나오는가 | 방법 |
|---|---|---|
| ReLU 1층 | ✅ 공짜 (항등식) | `[A_r\|b_r] = base + Σ r_i·c_i` — **STEP 13** |
| ReLU 2층 | ❌ 깨짐 (잔차 0.168) | — **STEP 14** |
| ReLU 2층, 출구 (A) | ✅ 학습으로 | APD → SPD → VPD (**Stage 2**) |
| **bilinear** | ✅ **공짜 (고유분해)** | 3차 텐서, 데이터 불필요 (**새 Stage**) |

**즉 "깊이가 공짜 분해를 없앤다" 는 STEP 14 의 결론에 출구가 둘이고,
Stage 2 는 (A), 새 방향은 (B) 다.** 억지로 갖다 붙이는 게 아니라 같은 질문의 두 답이다.

### 13.5 우리가 가진 자산이 그대로 쓰인다

- MNIST 단층 모델 — 논문의 §4 세팅과 **같은 규모**다 (shallow FFN, MNIST/Fashion-MNIST)
- 적대적 예제로 인과 검증 — 우리 **Stage 5(a) 개입 실험**과 같은 형태
- 학습 코드·데이터 파이프라인 — 활성함수만 바꾸면 된다
- STEP 8~11 의 방법론(앵커·손잡이·대조군) — **그대로 적용**한다.
  고유분해에도 손잡이가 있다(어느 출력 방향으로 자를지, 몇 개를 볼지, 정규화 강도).
  논문 자신이 *"regularization 이 피처 품질에 중요하다"* 고 적어뒀다.
