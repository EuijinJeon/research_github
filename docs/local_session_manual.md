...
## 2. 받아야 할 자료 — 우선순위 순  ✅ **A·B 전부 확보 완료 (2026-08-19)**

> `external/papers/` 에 PDF 5개 + `pdftotext -layout` 로 뽑은 `.txt` 5개가 있다.
> 재확보가 필요하면 아래 한 줄이면 된다:
> ```bash
> mkdir -p external/papers && cd external/papers
> for u in 2011.04041 2211.12312 1906.00904 1410.1165 1802.06259; do
>   curl -sL -o "$u.pdf" "https://arxiv.org/pdf/$u" && pdftotext -layout "$u.pdf"
> done
> ```
...
## 3. 각 자료에서 뽑을 것 — 체크리스트  ✅ **2026-08-19 전부 완료**

> **완료 보고.** 논문 5편(A1~A3, B1~B2)을 `external/papers/`에 받아 전부 읽었다.
> 아래 체크박스는 답이 어디에 적혔는지의 색인으로 남긴다. 결과 요약은 `prior_work.md` §8.
>
> **뒤집힌 것 셋:** ① Frobenius는 각주의 지나가는 제안일 뿐 쓰인 적 없음 (§2)
> ② 절편 가중치 λ는 Aletheia 논문에 없음, 코드 전용 (§2) ③ τ·refit이 MNIST를 무효화한다는
> 추론이 틀렸음 — FicoHeloc이 R/N=0.94로 이미 같은 체제 (§4.2)
> **덤으로 얻은 것 셋:** ④ 부호벡터 해밍 거리라는 네 번째 척도 (§3.3)
> ⑤ 증분 정확 계수법 → Q4의 h 제한 해소 (§4.3) ⑥ 대조군 둘: 랜덤 배정 조견표, 암기 모델 (§6.1)

**읽고 나서 답을 `docs/prior_work.md`의 해당 절에 적고, 근거 등급을 [요약] → [원문]으로 올린다.**
답이 우리 잠정안과 다르면 `docs/open_questions.md`도 함께 고친다.

### ⚠️ 이 체크리스트의 결함 — 다음 조사 때 반드시 고칠 것

아래 항목들은 전부 **D1~D4를 풀기 위한 전술적 질문**이다
("Frobenius를 썼나", "HDBSCAN을 뭐에 돌렸나", "UMAP을 쓰나").
**논문 자신의 결론과 한계를 묻는 항목이 하나도 없다.**

그 결과 polytope lens 원문에서 **방향 판단에 가장 중요한 문단**(클러스터링 = `k=1`,
*"decomposable description을 찾는 데 명백히 최적이 아니다"*)을 **통째로 놓쳤다.**
체크리스트에 없었으니 안 찾은 것이다. `prior_work.md` §10 참조.

**교훈: 전술적 질문만 담은 체크리스트는 전략적 내용을 걸러낸다.**
다음 조사 체크리스트에는 자료마다 아래 셋을 **먼저** 넣는다:

- [ ] **저자들이 스스로 밝힌 한계는 무엇인가** (보통 Discussion / Future work 절)
- [ ] **저자들이 "다음에 하라"고 한 것은 무엇인가** — 그게 실제로 됐는지도 검색한다
- [ ] **이 논문 이후 이 방향이 어떻게 됐는가** — 인용 검색. 우리는 이걸 안 해서
      "이 계열은 2020년경 멈췄다"고 **검증 없이 단정했다** (§9에서 반박됨)

### A1. Aletheia 논문 (2011.04041)

- [x] 병합할 LLM 쌍을 고르는 **거리 정의가 논문에 수식으로 나오는가?** 계수만인가 절편 포함인가
- [x] 절편(intercept)에 **가중치를 곱하는가**. 곱한다면 값은 어떻게 정했나 → **D1의 λ에 직결**
- [x] 클러스터링 전에 **표준화(standardize)를 하는가**
- [x] `n_neighbors`(연결성 그래프) 기본값의 근거. 예제는 `LLM 수 × 0.01`
- [x] 병합 후 **refit**을 하는 이유를 뭐라고 설명하는가. refit 없는 버전과 비교한 표가 있는가
- [x] 실험한 데이터셋의 **영역당 평균 샘플 수** — 우리 R/N과 비교하기 위해
- [x] `Pruner`(topk core + 최근접)와 `Merger`(agglomerative)의 **성능 격차 표가 있는가** → **D3에 직결**
- [x] `flatten`(병합 결과를 1층 망으로)의 성능이 원본 대비 얼마나 떨어지는가 → **Q1에 직결**

### A2. polytope lens (2211.12312)

- [x] Frobenius norm 유사도를 **실제 실험에서 썼는가, 정의만 하고 활성 위에서 클러스터링했는가**
      → 후자라면 **"Frobenius를 실제로 쓴 선행연구는 없다"** 가 되고, D1의 서술을 고쳐야 한다
- [x] HDBSCAN을 무엇 위에 돌렸나 (활성 / spline code / affine map)
- [x] UMAP 같은 차원축소를 먼저 하는가
- [x] "monosemantic polytope"을 **어떻게 검증**했나 (사람 라벨? 자동?)
- [x] 폴리토프 개수 규모를 어떻게 다뤘나 (서브샘플? 층별?)

### A3. Hanin & Rolnick (1906.00904)

- [x] 직선/평면을 **몇 개, 어떻게** 뽑나 (데이터 두 점 잇기? 랜덤 방향? 격자?)
- [x] 교차 횟수를 어떻게 세나 (스텝 수, 수치 허용오차)
- [x] 보고 단위가 무엇인가 (단위 길이당 영역 수? 총 개수?)
- [x] **학습 전후로 영역 밀도가 어떻게 변하나** → Q3(정규화가 ε-path를 낙관적으로 만드는가)의 대조군
- [x] Q4(격자 실측 10 vs 공식 11 등, 격차가 h에 따라 커짐)에 대한 설명이 있는가
      — 학습이 유닛을 정렬시켜 퇴화 배치를 만든다는 관찰이 이 논문에 있는지

### B1. Srivastava 2015

- [x] README가 "활성 코드 클러스터링"이라 적어뒀는데, **실제로 클러스터링인가 검색용 해싱인가**
- [x] 활성 패턴 간 거리를 뭘로 쟀나 (Hamming?)
- [x] "데이터 포인트에서 영역을 샘플링" — `concepts.md` §1.4가 이 논문을 인용하는데, **맞는 인용인지 확인**

### B2. Chu et al. 2018

- [x] README 목록에 있는데 이번에 아무것도 확인 못 함. **읽고 한 줄 요약을 남기거나, 목록에서 뺀다**

---

## 4. 조사가 끝나면 확정할 것 (결정 템플릿)  ← **여기부터. 조사는 끝났다**
...
- [x] **D1** — logit 기준 유지. `[A_r | λ·b_r]`의 **λ를 무엇으로 둘지** (1.0 고정? 민감도 측정?)
- [x] **D2** — agglomerative(average) + kNN 연결성 + **확률적 선택 γ=0.2**.
      **refit을 쓸지 말지** (2D는 가능, MNIST는 영역당 1점이라 불가 → 세팅별로 다르게 갈지)
- [x] **D3** — `Pruner` 방식(topk core + 최근접)으로 **서브샘플 없이** 6만 개 전부 처리.
      2D에서 Merger 대비 격차를 먼저 측정
- [x] **D4** — **KL 하나를 기준으로.** 정확도는 보조 표시, 선택 기준 아님
- [x] **곡선** — K=R, K=1 양 끝점 + **랜덤 초기화 모델 대조군** 포함
- [x] **보고** — 손잡이(α 또는 K, γ, linkage, λ) 없이 "K개로 압축된다"고 쓰지 않기
...
2. D1 측정  2D 실모델에서 네 척도(logit / Frobenius / cosine / 부호벡터 해밍)가
            같은 답을 주는지. λ 민감도도 여기서 (λ=1 이 재현 기준선)
3. D2 측정  2D에서 (a) greedy vs 확률적 선택(γ=0.2) 곡선 격차
                    (b) Merger vs Pruner 격차
                    (c) 소형 모델(h=6~8, 조각 20~30개)에서 전수탐색 vs greedy
                    (d) 연결성 제약 있음/없음 — 제약은 입력공간 μ_P 기준임에 주의
4. ε-path   2D 4개 모델에 대해 K=R → K=1 전체 곡선 (KL 기준) + 대조군
            대조군: K=R·K=1 종점 / 랜덤 배정 조견표 / 랜덤 초기화 모델 / 암기 모델
5. MNIST    Pruner 방식으로 6만 개 → K개. refit 있음/없음 두 곡선
```

> **2026-08-19 원문 확인 반영분** (바뀐 곳): 2번의 척도가 셋 → **넷**(해밍 추가),
> 3번에 **(d) 연결성 제약** 추가, 4번의 대조군이 하나 → **넷**,
> 5번은 ~~"refit 불가"~~ → **refit 가능**(refit은 영역이 아니라 클러스터 단위).
> 근거는 `prior_work.md` §3.3 / §3.1 / §6.1 / §4.2.
